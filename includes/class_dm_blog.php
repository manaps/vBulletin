...
class vB_DataManager_Blog_Abstract extends vB_DataManager
{
	...
	*/
	function post_save_each_blogtext($doquery = true)
	{
		...
		{
			$this->store_ipaddress($blogtextid, $contenttypeid);
		}
	...
	*/
	function store_ipaddress($contentid = 0, $contenttypeid = 0, $rectype = 'content')
	{
		...
		$ipid = $ipman->save();
		$return = $ipman->update_content('blog_text', 'blogtextid', $ipid, $contentid);
		unset($ipman);
	...
	public function delete($doquery = true)
	{
		if ($blogid = $this->existing['blogid'])
		{
			...
			if ($this->info['hard_delete'])
			{
				...
				if (!empty($ipids))
				{
					$db->query_write("DELETE FROM " . TABLE_PREFIX . "ipdata WHERE ipid IN (" . implode(',', $ipids) . ")");
				}
	...
	function pre_save($doquery = true)
	{
		...
		if (!$this->condition AND $this->fetch_field('state') == 'visible' AND !$this->info['skip_akismet'])
		{
			...
			if (!empty($akismet_key))
			{
				...
				$akismet_data['comment_content'] = $this->fetch_field('title') . ' : ' . $this->fetch_field('pagetext');
	...
	public function delete($doquery = true)
	{
		if ($blogtextid = $this->existing['blogtextid'])
		{
			...
			if ($this->info['hard_delete'])
			{
				...
				$db->query_write("
					DELETE " . TABLE_PREFIX . "blog_text, " . TABLE_PREFIX . "blog_textparsed, " . TABLE_PREFIX . "ipdata
					FROM " . TABLE_PREFIX . "blog_text
					LEFT JOIN " . TABLE_PREFIX . "ipdata ON (" . TABLE_PREFIX . "blog_text.ipaddress = " . TABLE_PREFIX . "ipdata.ipid)
					LEFT JOIN " . TABLE_PREFIX . "blog_textparsed ON (" . TABLE_PREFIX . "blog_textparsed.blogtextid = " . TABLE_PREFIX . "blog_text.blogtextid)
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 17:39, Sat Aug 3rd 2019 : $Revision: 93970 $
