	...
	if (!isset($blogcache["$blogid"]) OR !$usecache)
	{
		...
		$blogcache["$blogid"] = $vbulletin->db->query_first("
			SELECT
				...
				blog_text.pagetext, blog_text.allowsmilie, ipdata.ipid AS ipaddress, blog_text.reportthreadid, ipdata.ip AS blogipaddress, blog_text.htmlstate,
				...
				" . (($vbulletin->userinfo['userid']) ? ", blog_read.readtime AS blogread, blog_userread.readtime  AS bloguserread" : "") . "
			...
			LEFT JOIN " . TABLE_PREFIX . "ipdata AS ipdata ON (blog_text.ipaddress = ipdata.ipid)
		...
		$blogcache["$blogid"]['categorycache'] = !empty($blogcache["$blogid"]['categorycache']) ? vb_unserialize($blogcache["$blogid"]['categorycache']) : array();
		$blogcache["$blogid"]['sidebar'] = !empty($blogcache["$blogid"]['sidebar']) ? vb_unserialize($blogcache["$blogid"]['sidebar']) : array();
		$blogcache["$blogid"]['sidebar_customblocks'] = !empty($blogcache["$blogid"]['blocktext']) ? vb_unserialize($blogcache["$blogid"]['blocktext']) : array();
		$blogcache["$blogid"]['custompages'] = !empty($blogcache["$blogid"]['custompages']) ? vb_unserialize($blogcache["$blogid"]['custompages']) : array();
	}
...
function fetch_blog_textinfo($blogtextid)
{
	...
	if (!isset($blogtextcache["$blogtextid"]))
	{
		$blogtextcache["$blogtextid"] = $vbulletin->db->query_first("
			SELECT blog_text.*, ipdata.ip AS ipaddress,
				blog_editlog.userid AS edit_userid, blog_editlog.dateline AS edit_dateline, blog_editlog.reason AS edit_reason, blog_editlog.username AS edit_username
...
function fetch_blog_tagcloud($type = 'usage', $links = false, $userid = 0)
{
	...
	if ($userid AND $type == 'usage')
	{
		...
		$cloud = vb_unserialize($userinfo['tagcloud']);
	}
	else
	{
...
function construct_usercss_blog(&$userinfo, &$show_usercss_switch)
{
	...
	{
		if ($csscolors = vb_unserialize($userinfo['blog_csscolors']))
		{
