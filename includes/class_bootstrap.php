...
class vB_Bootstrap
{
	...
	*/
	public function process_templates()
	{
		...
		$templater = vB_Template::create('headinclude');
			...
			$templater->register('user_default_style_type', isset($vbulletin->stylecache['mobile'][$vbulletin->userinfo['realstyleid']]) ? 'mobile' : 'standard');
	...
	*/
	public function load_show_variables()
	{
		...
		$loggedout = false;
		if (defined('THIS_SCRIPT') && THIS_SCRIPT == 'login' AND $_REQUEST['do'] == 'logout' AND $vbulletin->userinfo['userid'] != 0)
		{
		...
		$show['dst_correction'] = (
			defined('THIS_SCRIPT') && THIS_SCRIPT != 'register' AND $vbulletin->options['bbactive']
	...
	*/
	protected function fetch_style_record($force_styleid = 0)
	{
		...
		$userselect = defined('THIS_SCRIPT') && THIS_SCRIPT == 'css' ? true : false;
...
class vB_Bootstrap_Forum extends vB_Bootstrap
{
	...
	*/
	public function load_show_variables()
	{
		...
		if (defined('THIS_SCRIPT') && THIS_SCRIPT == 'showthread' AND $threadinfo['threadid'])
		{
