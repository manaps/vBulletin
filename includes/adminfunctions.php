...
*/
function print_textarea_row($title, $name, $value = '', $rows = 4, $cols = 40, $htmlise = true, $doeditbutton = true, $direction = '', $textareaclass = false, $isAce = false)
{
    global $vbphrase, $vbulletin;
    ...
    if ($isAce) {
        $vbulletin->textarea_id = 'aceCodeEditor';
    }
    ...
    print_label_row(
        ...
        "<div id=\"ctrl_$name\" style=\"position: relative;" . (($isAce) ? ' height: 500px;' : '') . "\"><" . (($isAce) ? 'div' : 'textarea') . " name=\"$name\" id=\"{$vbulletin->textarea_id}\"" . iif($textareaclass, " class=\"$textareaclass\"") . " rows=\"$rows\" cols=\"$cols\" wrap=\"virtual\" dir=\"$direction\" tabindex=\"1\"" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot;\"") . ">" . iif($htmlise, htmlspecialchars_uni($value), $value) . "</" . (($isAce) ? 'div' : 'textarea') . ">$resizer</div>" . (($isAce) ? '<textarea name="' . $name . '" id="rawEditor" style="display: none;" ></textarea>' : ''),
        '', 'top', $name
    );
}
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 17:39, Sat Aug 3rd 2019 : $Revision: 92990 $
