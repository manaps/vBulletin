...
abstract class vB_StyleVar
{
	...
	public function print_editor($masterstyleid)
	{
		...
		if ($this->inherited == -1 OR $this->inherited == -2)
		{
			...
			$header .= (is_countable($header) && count($header) == 1 ? '<span class="smallfont">' : '') . construct_link_code($vbphrase['revert'], "stylevar.php?" . $vbulletin->session->vars['sessionurl'] . "do=confirmrevert&amp;dostyleid=" . $vbulletin->GPC['dostyleid'] . "&stylevarid=" . $this->stylevarid . "&rootstyle=" . $this->inherited);
		}
