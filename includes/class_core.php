...
if (! defined('THIS_SCRIPT')) {
    // if we have no THIS_SCRIPT value, set it to something random
    define('THIS_SCRIPT', 'noteverused999');
}
	...
	*/
	function __construct(&$registry, $sessionhash = '', $userid = 0, $password = '', $styleid = 0, $languageid = 0)
	{
		...
		if ($this->created == false)
		{
			...
			{
				$this->set('location', (defined('WOLPATH') ? WOLPATH : $this->registry->wolpath));
			}
...
class vB_Shutdown
{
	...
	*/
	...
	{
		if (is_countable($this->callbacks))
		{
	...
	*/
	...
	protected function render_output($suppress_html_comments = false)
	{
		...
		try {
		    ...
		    // TODO: Log these errors to a file instead of just printing them
            $error = sprintf('Template error in <strong>%s</strong>: %s in %s on line %s', $this->template, $e->getMessage(), $e->getFile(), $e->getLine());
            file_put_contents(ROOT_DIRECTORY . 'errors/template_errors.log', $error);
        }
...
class vB_Template_Runtime
{
	...
	public static function runMaths($str)
	{
		...
		//mixed units.
		if (is_countable($units_found) && count($units_found) > 1)
		{
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 17:39, Sat Aug 3rd 2019 : $Revision: 93876 $
