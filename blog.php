...
if ($_REQUEST['do'] == 'blog')
{
	...
	do
	{
		...
		$comments = $db->query_read("
			SELECT SQL_CALC_FOUND_ROWS blog_text.*, ipdata.ip AS blogipaddress,
...
if ($_REQUEST['do'] == 'comments')
{
	...
	do
	{
		...
		$comments = $db->query_read("
			SELECT SQL_CALC_FOUND_ROWS
				blog_text.username AS postusername, ipdata.ip AS blogipaddress, blog_text.state, blog_text.dateline, blog_text.pagetext, blog_text.allowsmilie, blog_text.blogtextid, blog_text.title,
...
//vbstop($blogtextinfo);
		...
		$ipaddress = ($blogtextinfo['ipaddress'] ? $blogtextinfo['ipaddress'] : '');
	}
	else
	{
		$bloginfo = verify_blog($blogid);
		$ipaddress = ($bloginfo['blogipaddress'] ? $bloginfo['blogipaddress'] : '');
	}
