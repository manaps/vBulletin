 ...
 * @version $Revision: 92541 $
 * @since $Date: 2017-01-22 16:11:25 -0800 (Sun, 22 Jan 2017) $
...
class vBBlog_Search_IndexController_BlogComment extends vB_Search_IndexController
{
	...
	{
		$indexer = vB_Search_Core::get_instance()->get_core_indexer();
		if (is_callable(array($indexer, 'delete_group')))
		{
	...
	public function group_data_change($groupid)
	{
		global $vbulletin;
		$indexer = vB_Search_Core::get_instance()->get_core_indexer();
		if (is_callable(array($indexer, 'group_data_change')))
		{
	...
	private function make_query($filter)
	{
		...
		return "
			SELECT blog.blogid, blog.userid AS blog_userid, blog.dateline AS blog_dateline,
				...
				blog_text.dateline, blog_text.blogtextid, ipdata.ip AS ipaddress, blog_text.pagetext,
			...
			FROM " . TABLE_PREFIX . "blog AS blog 
				...
				LEFT JOIN " . TABLE_PREFIX . "user AS blog_user ON blog.userid = blog_user.userid 
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 17:39, Sat Aug 3rd 2019 : $Revision: 92541 $
|| # $Date: 2017-01-22 16:11:25 -0800 (Sun, 22 Jan 2017) $
|| ####################################################################
\*======================================================================*/
