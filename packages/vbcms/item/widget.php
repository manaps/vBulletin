	...
	protected function applyLoad($result, $load_query)
	{
		if (self::QUERY_CONFIG == $load_query)
		{
			...
			while ($cvar = vB::$db->fetch_array($result))
			{
				if ($cvar['instance'])
				{
					$instance_config[$cvar['name']] = ($cvar['serialized'] ? vb_unserialize($cvar['value']) : $cvar['value']);
				}
				else
				{
					$node_config[$cvar['name']] = ($cvar['serialized'] ? vb_unserialize($cvar['value']) : $cvar['value']);
				}
			}
