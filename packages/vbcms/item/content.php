...
class vBCms_Item_Content extends vB_Item_Content
{
	...
	* ***/
	...
	protected function loadCacheInfo($info)
	{
		...
		{
			$this->parents = vb_unserialize($info['parents']);
		}

		if (isset($info['config']))
		{
			$this->config = vb_unserialize($info['config']);
		}
	...
	****/
		...
		$publicpreview = false, $comments_enabled = false, $pagination_links = 1)
	{
		if ($this->canPublish())
		{
			...
			/* Getdate is a most annoying function for us. 
			It takes a Unix time stamp and converts it to server local time. 
			We need to compensate for the difference between server time (date('Z')) and usertime */
			$offset = vBCms_ContentManager::getTimeOffset(vB::$vbulletin->userinfo) - intval(date('Z', TIMENOW));
			...
			if (!$this->getAssociatedThreadId() AND
				is_callable(array($this, 'getPostId')) AND ($this->getPostId() > 0))
			{
			...
			if (is_callable(array($this, 'getAllComments')))
			{
				$pub_view->showAllComments = (is_callable(array($this, 'getPostId')) AND ($this->getPostId() > 0));
			...
			if (is_callable(array($this, 'getPostId')) AND is_callable(array($this, 'getKeepThread')) AND $this->getPostId())
			{
