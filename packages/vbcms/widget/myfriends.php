 ...
 * @version $Id: myfriends.php 92140 2016-12-31 04:26:15Z pmarsden $
 * @access public
 */
class vBCms_Widget_myFriends extends vBCms_Widget
{
	...
	private function getResults($config)
	{
		...
		if ($cache_data)
		{
			...
			if (is_callable(array($controller, 'create_array')))
			{
				$results = $controller->create_array($cache_data['ids']);
			}
			else if(is_callable(array($controller, 'create_item')))
			{
				$results = array();
