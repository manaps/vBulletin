...
if ($_REQUEST['do'] == 'modify')
{
	...
	if ($vbulletin->options['cp_collapse_forums'] != 2)
	{
		...
		if (is_array($vbulletin->forumcache))
		{
			...
			{
				$modcount = is_countable($imodcache["$forum[forumid]"]) ? sizeof($imodcache["$forum[forumid]"]) : 0;
				if ($modcount)
				{
				...
				if (!$vbulletin->options['cp_collapse_forums'] OR $forum['forumid'] == $expanddata['forumid'] OR in_array($forum['forumid'], $expanddata['parentids']))
				{
					...
					$mods = array('no_value' => $vbphrase['moderators'].' (' . (is_countable($imodcache["$forum[forumid]"]) ? sizeof($imodcache["$forum[forumid]"]) : 0) . ')');
