...
if ($_REQUEST['action'] == 'options' OR empty($_REQUEST['action']))
{
	...
	if ($_REQUEST['do'] == 'options' AND !empty($vbulletin->GPC['varname']))
	{
		...
		else if ($group = $db->query_first_slave("SELECT varname, grouptitle FROM " . TABLE_PREFIX . "setting WHERE varname = '" . $db->escape_string($vbulletin->GPC['varname']) . "'"))
		{
			...
			exec_header_redirect('registration.php?' . $vbulletin->session->vars['sessionurl_js'] . "do=options&dogroup=$group[grouptitle]#$group[varname]");
		}
	}
	...
	print_cp_header($vbphrase['dbtech_vbanalytics_settings']);
	...
	$settings = $db->query_read_slave("
		...
		WHERE settinggroup.product LIKE 'dbtech_registration%'
	...
	print_form_header('registration', 'options', 0, 1, 'groupForm', '90%', '', 1, 'get');
	...
	else // show the small list with selected setting group(s) options
	{
		...
		print_form_header('registration', 'options', false, true, 'optionsform', '90%', '', true, 'post" onsubmit="return count_errors()');
...
if ($_REQUEST['action'] == 'dooptions')
{
	...
	if (!empty($vbulletin->GPC['setting']))
	{
		...
		define('CP_REDIRECT', 'registration.php?do=options&amp;dogroup=' . $vbulletin->GPC['dogroup'] . '&amp;advanced=' . $vbulletin->GPC['advanced']);
