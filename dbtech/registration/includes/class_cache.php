...
class REGISTRATION_CACHE
{
	...
	* @public	string
	*/	
	public static $prefix 			= 'dbtech_registration_';
	...
	*/	
	protected static $_tables		= array(
		...
		'criteria'		=> '',
		'field' 		=> '',
		'instance'		=> 'ORDER BY priority ASC',
		'redirect' 		=> '',
		'section'		=> '',
	);
	...
	*/
	public static function build($type)
	{
		...
		if (!is_object(REGISTRATION::$db))
		{
			// Ensure this doesn't error on upgrades
			return false;
		}

		REGISTRATION::$db->hideErrors();
		$data = REGISTRATION::$db->fetchAllKeyed('
			SELECT :dbtype.*
		...
		// Set the data
		REGISTRATION::$cache[$field_short] = $data;
		
		foreach ((array)REGISTRATION::$cache[$field_short] as $id => $arr)
		{
			foreach ((array)REGISTRATION::$unserialize[$field_short] as $key)
			{
				// Do unserialize
				REGISTRATION::$cache[$field_short][$id][$key] = @unserialize(stripslashes($arr[$key]));
			}
		}
	}
	...
	*/
	public static function read($cacheType, $key)
	{
		...
		// Fetch the vBO data
		$_data = vb_optimise::$cache->get('dbtech.registration.' . $key);
		...
		if (is_array($_data) AND TIMENOW < $_data['time'])
		{
			...
			vb_optimise::stat($i);
			vb_optimise::report('Fetched dbtech.registration.' . $key . ' from cache successfully.');
	...
	*/
	public static function write($data, $cacheType, $key)
	{
		...
		if ($data !== false)
		{
			// Write the vBO data
			$_data = array(
				'time'	=> TIMENOW + (self::$vbulletin->options['vbo_cache_registration' . $cacheType] * 3600),
				'cache'	=> $data,
			);
		}

		// Write the cache
		vb_optimise::$cache->set('dbtech.registration.' . $key, $_data);
		vb_optimise::report('Cached dbtech.registration.' . $key . ' successfully.');	
	...
	*/	
	protected static function _canCache($cacheType = '')
	{
		...
		if (!isset(self::$vbulletin->options['vbo_cache_registration' . $cacheType]))
		{
			// vBO's version is too old
			return false;
		}

		if (!self::$vbulletin->options['vbo_cache_registration' . $cacheType])
		{
