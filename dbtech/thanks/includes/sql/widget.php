...
if ((bool)self::$vbulletin->products['vbblog'] AND !(self::$vbulletin->options['dbtech_thanks_disabledintegration'] & 1))
{
	$SQL[] = '
		SELECT
			...
			0 AS forumid,
			entry_blog.*
		FROM (SELECT * FROM $dbtech_thanks_entry WHERE varname :whereCond AND contenttype = \'blog\' ORDER BY entryid DESC LIMIT :limit) AS entry_blog
...
if ((bool)self::$vbulletin->products['dbtech_downloads'] AND !(self::$vbulletin->options['dbtech_thanks_disabledintegration'] & 128))
{
	$SQL[] = '
		...
		FROM (SELECT * FROM $dbtech_thanks_entry WHERE varname :whereCond AND contenttype = \'vbdownloads_download\' ORDER BY entryid DESC LIMIT :limit) AS entry_vbdownloads_download
