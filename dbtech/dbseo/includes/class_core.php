	...
	*/
	public static function init($lateInit = false)
	{
		...
		*/
		...
		{
			die('{"version":"2.0.42","pro":"' . ($isPro ? 'true' : 'false') . '","vbversion":"' . self::$config['templateversion'] . '"}');
		}
	}
	...
	public static function processContent(&$content, $isFinal = false)
	{
		...
		if (
			DBSEO_BASEDEPTH
			AND preg_match('#<base href="([^\"]*)#i', $content, $matches)
			AND preg_replace('#/[^/]*$#', '', $matches[1]) == self::$config['_bburl']
		)
		{
