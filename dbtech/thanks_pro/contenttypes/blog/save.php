...
)))
{
	// Invalid post id
	THANKS::outputXML(array(
		'error' => $vbphrase['dbtech_thanks_invalid_blogid'] . ': ' . $contentid
	));
}
...
if ((int)$vbulletin->options['dbtech_thanks_disabledbuttons_blog'] & (int)$button['bitfield'])
{
...
if ((int)$post['disabledbuttons_blog'] & (int)$button['bitfield'])
{
	// Button was disabled for this thread
	THANKS::outputXML(array(
		'error' => $vbphrase['dbtech_thanks_button_disabled_blog']
	));
}

if ((int)$post['disabledbuttons_blogtext'] & (int)$button['bitfield'])
{
	// Button was disabled for this post
	THANKS::outputXML(array(
		'error' => $vbphrase['dbtech_thanks_button_disabled_blogtext']
	));
}

// Refresh AJAX post data
$excluded = THANKS::refreshAjaxPost($contentid, 'blog');
...
list($entries, $actions) = THANKS::processDisplay($noticeforum, $excluded, $post, array('dbtech_thanks_disabledbuttons' => $post['disabledbuttons_blog'], 'firstpostid' => $post['firstpostid']), 'blog');
